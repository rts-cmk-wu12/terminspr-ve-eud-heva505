...
export async function // API-kald / نداء API
getActivities(q) {
  await wait();
  ...
  return activities.filter(
    (a) =>
      a.name?.toLowerCase().includes(s) || a.weekday?.toLowerCase().includes(s)
  );
}
export async function getActivitiesByInstructor(instructorId) {
  await wait();
  return activities.filter(
    (a) => Number(a.instructorId) === Number(instructorId)
  );
}
...
export async function enroll(userId, activityId) {
  ...
  const exists = enrollments.some(
    (e) => e.userId === Number(userId) && e.activityId === Number(activityId)
  );
